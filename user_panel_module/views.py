...
from user_panel_module.forms import EditProfileModelForm, ChangePasswordModelForm, EditUserFavoriteForm
...
@method_decorator(login_required, name='dispatch')
class UserFavoriteCategories(View):
    def get(self, request: HttpRequest):
        current_user = User.objects.filter(id=request.user.id).first()
        edit_form = EditUserFavoriteForm(instance=current_user)
        context = {
            'form': edit_form,
            'current_user': current_user
        }
        return render(request,'user_panel_module/user_favorite_categories.html',context)

    def post(self, request: HttpRequest):
        current_user = User.objects.filter(id=request.user.id).first()
        fav_form = EditUserFavoriteForm(request.POST, instance=current_user)
        if fav_form.is_valid():
            fav_form.save(commit=True)
        context = {
                'form': fav_form,
                'current_user': current_user
            }
...
def user_basket(request: HttpRequest):
    ...
    total_amount = current_order.get_calculate_total_price()
